<template>
  <div
      class="min-h-screen bg-gradient-to-tr from-slate-950 via-indigo-950 to-slate-950 flex flex-col text-slate-100 relative">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <span
                class="ml-1 text-[10px] font-bold text-indigo-400 px-1.5 py-0.5 rounded bg-indigo-950/80 border border-indigo-800/40">极光网盘</span>
          </div>
        </div>

        <div class="flex items-center gap-3">
          <button v-if="isLogin" @click="handleLogout"
                  class="px-4 py-2 rounded-xl text-xs font-semibold bg-rose-500/10 hover:bg-rose-500 text-rose-300 hover:text-white border border-rose-500/20 transition-all shadow-md shadow-rose-950/20">
            安全登出
          </button>
        </div>
      </div>
