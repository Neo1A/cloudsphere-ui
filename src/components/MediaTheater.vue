<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
    <div class="max-w-4xl w-full rounded-2xl border border-slate-800 bg-slate-900/90 backdrop-blur-box overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
      ...
      <div class="p-4 border-b border-slate-800/80 flex items-center justify-between">
        ...
        <button @click="$emit('onCloseTheater')" class="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white">
          <X class="h-4 w-4" />
        </button>
      </div>

      <div class="flex-1 bg-black p-2 flex items-center justify-center min-h-[400px]">
        <video v-if="resolvedType === 'video'" :src="authenticatedUrl" controls autoplay class="max-h-[60vh] w-full object-contain"></video>
        <audio v-else-if="resolvedType === 'audio'" :src="authenticatedUrl" controls autoplay class="w-full max-w-lg" />
        <img v-else-if="resolvedType === 'image'" :src="authenticatedUrl" class="max-h-[60vh] object-contain rounded-lg shadow-2xl" />

        <DocumentViewer v-else-if="resolvedType === 'office'" :file="file" :token="token" />

        <div v-else-if="resolvedType === 'markdown'" class="w-full h-[65vh] overflow-y-auto px-8 py-6 bg-slate-950/60 rounded-xl text-left">
        ...
        <div v-else-if="resolvedType === 'text'" class="w-full max-h-[65vh] flex flex-col bg-slate-950 rounded-xl overflow-hidden text-left border border-slate-800">
          <div class="flex items-center justify-between px-4 py-2 bg-slate-900/50 border-b border-slate-800">
            ...
            <button @click="copyTextToClipboard(textContent)" class="text-[10px] font-bold text-slate-300 hover:text-white bg-slate-800 px-3 py-1.5 rounded-lg">复制</button>
          </div>
...
import { X, FileCode, FileVideo, FileAudio, FileImage, FileText, FileSpreadsheet, FileArchive, ShieldAlert, File, HelpCircle } from 'lucide-vue-next'
